...
class CliffWalkingBase(Env):
  def __init__(self, n_rows=4, n_cols=12, x_ini=0, y_ini=0, x_goal=11, y_goal=0):
    super(CliffWalkingBase, self).__init__()

    self.action_space = spaces.Discrete(4)

    self.observation_space = spaces.Box(low=np.array([0, 0]), high=np.array([n_cols - 1, n_rows - 1]), shape=(2,), dtype=np.int)

    self.cliff = list(range(1, n_cols - 1))
    ...
    for y in range(self.n_rows):
      for x in range(self.n_cols):
        if x == self.x_pos and y == self.n_rows - self.y_pos - 1:
          ...
          print('|G', end='')
        elif x in self.cliff and y == self.n_rows - 1:
          print('|C', end='')
        else:
          print('|_', end='')
      print('|')

class CliffWalking4Actions(CliffWalkingBase):
  def __init__(self, n_rows=4, n_cols=12, x_ini=0, y_ini=0, x_goal=11, y_goal=0):
    super(CliffWalking4Actions, self).__init__(n_rows, n_cols, x_ini, y_ini, x_goal, y_goal)
  ...
  def step(self, action):
    ...
    if action == 0: # left
      ...
      self.y_pos = min(self.y_pos + 1, self.n_rows - 1)
    elif action == 2: # right
    ...
    elif self.y_pos == 0 and self.x_pos in self.cliff:
      reward = -100
      self.x_pos = self.x_ini
      self.y_pos = self.y_ini
      done = False
